...
**Open decisions for the human (before Phase 0):** catalog scope (bright-star only
vs. + faint Gaia); Jacobian method (FD-first vs. straight to geodesic deviation);
diffuse map (keep a small Milky-Way equirect vs. point stars only first); color
fidelity (blackbody-from-B−V vs. catalog RGB). Nothing is implemented until these
are settled and the Phase-0 SKILL.md formulas are approved.
