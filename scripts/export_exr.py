...
def write_rgbaz_exr(filename: str, beauty_rgb: np.ndarray, depth_z: np.ndarray) -> None:
    ...
    pixels = np.concatenate(
        [np.ascontiguousarray(beauty_rgb, dtype=np.float32),
         np.ascontiguousarray(depth_z, dtype=np.float32)], axis=2
    )
...
def main(argv=None):
    parser = argparse.ArgumentParser(description="Multi-channel RGBAZ EXR export (Pipe A + Pipe B + Z).")
    parser.add_argument("--frame", type=int, default=0,
                        help="0-based index into camera_matrix.json (default 0).")
    ...
    with open(cam_path, "r", encoding="utf-8-sig") as fh:
        frames = json.load(fh)
    ...
    print(f"rendering {width}x{height}  frame={args.frame}  disk={'off' if args.no_disk else 'on'}"
    ...
    if args.motion_blur:
        ...
        beauty, depth = tr.render_beauty_frame(
            cfg, cam, width, height,
            with_disk=not args.no_disk, lod_enabled=True, return_depth=True)
    beauty = np.nan_to_num(beauty)
    dt = time.time() - t0
    print(f"done in {dt:.1f}s  beauty[max={beauty.max():.4g}]  "
