...
def main(argv=None):
    ...
    print(f"camera: index {args.frame} (json frame={cam['frame']})  "
    ...
    t0 = time.time()
    hdr = tr.render_beauty_frame(cfg, cam, args.width, args.height,
                                 with_disk=not args.no_disk, lod_enabled=True)
    dt = time.time() - t0
    ...
    print(f"done in {dt:.1f}s  hdr[min={hdr.min():.4g} max={hdr.max():.4g} "
          f"mean={hdr.mean():.4g}]  non-black px={nonblack * 100:.1f}%")
    print(f"Doppler check: left-half mean lum={left:.4g}  right-half mean lum={right:.4g}  "
