"""Thumbnail preview renderer for the Kerr black hole (Pipe A only).

Traces one null geodesic per pixel through Kerr spacetime and produces a small
PNG that lets us visually confirm the **shadow** and **photon ring** before any
gas disk, starmap, or compositor work. There is intentionally no accretion disk
here (Pipe A only).
...
Usage
-----
    uv run python scripts/thumb.py --res 256 --frame 0
"""
...
def trace_pixel(r_cam, theta_cam, phi_cam, a, n, n_steps, d_lambda,
                r_max, r_plus):
    """Trace one photon and return (outcome, r_min).
    ...
    if r_last < 2.0:
        return "captured", r_min
    return "undecided", r_min
...
def render(cfg: dict, res: int) -> np.ndarray:
    bh = cfg["black_hole"]
    rcfg = cfg["render"]
    cam = cfg["camera"]
    th = cfg["thumb"]
    ...
    for py in range(res):
        ...
        for px in range(res):
            ...
            outcome, r_min = trace_pixel(
                r_cam, theta_cam, phi_cam, a, n,
                n_steps, d_lambda, r_max, r_plus,
            )
    ...
    # Tone map (clamp) + gamma encode for display.
...
def main(argv=None):
    ...
    pixels = render(cfg, res)
    out_path = Path(__file__).resolve().parent / "thumb_output.png"
