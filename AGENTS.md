...
| Coordinates | **Cartesian Kerr-Schild (t, x, y, z)** — active renderer path (spin axis = +z; CKS radius `r` is the BL radial coordinate, `z = r cosθ`). Boyer-Lindquist (t, r, θ, φ) is retired/history only. See SKILL.md PART II. |
| Metric signature | (− + + +) |
